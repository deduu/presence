...
class ImageProcessor:
    ...
    def get_image_paths(self, folder_path: Optional[str] = None) -> List[str]:
        ...
        if not os.path.isdir(search_folder):
            self.logger.error(
                f"Image folder '{search_folder}' does not exist.")
            return []
        ...
        self.logger.info(
            f"Found {len(image_paths)} images in '{search_folder}'.")
        return image_paths
    ...
    def process_multiple_images(self, image_paths: Optional[List[str]] = None) -> List[Dict]:
        ...
        self.logger.info(
            f"Successfully processed {len(results)} out of {len(image_paths)} images.")
        return results
    ...
    def annotate_faces(self,
        ...
        for (top, right, bottom, left), name in zip(face_locations, face_names):
            ...
            cv2.rectangle(image_bgr, (left, top),
                          (right, bottom), box_color, box_thickness)

            # Prepare label
            label = str(name) if name else "Unknown"

            # Calculate label position (above the box if possible, otherwise below)
            label_position = (left, top - 10) if top - \
                10 > 10 else (left, bottom + 25)
    ...
    def process_camera_stream(self, camera_index: int = 0) -> None:
        """
        Capture and process frames from a live camera feed.
        ...
        if not cap.isOpened():
            self.logger.error("Unable to access camera.")
            return

        self.logger.info("Starting live camera feed. Press 'q' to quit.")
        ...
        while True:
            ...
            face_locations, face_encodings = self.detect_faces(rgb_frame)

            # Annotate detected faces (all "Unknown" here for now)
            face_names = ["Unknown"] * len(face_locations)
            annotated_frame = self.annotate_faces(
                rgb_frame, face_locations, face_names)
