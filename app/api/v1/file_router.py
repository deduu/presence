...
class UploadPreviewResponse(BaseModel):
    ...
    processed_files: List[Dict[str, Any]] # Each dict contains preview_image_url, detections etc.
...
class ConfirmSaveRequest(BaseModel):
    ...
    face_detections: List[Dict[str, Any]] # Each dict should contain enough info for FaceService.save_confirmed_faces
...
@router.post("/upload-and-process", response_model=List[Dict]) # Adjust response model as needed
...
@router.post("/upload-and-process-multiple", response_model=List[Dict]) # Adjust response model as needed
async def upload_and_process_multiple_images(
    files: List[UploadFile] = File(...), # Changed from 'file' to 'files' and type to List[UploadFile]
    face_service: FaceService = Depends(get_face_service)
):
    if not files:
    ...
    for file in files:
        ...
        file_extension = os.path.splitext(file.filename)[1]
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S%f")
        unique_filename = f"uploaded_image_{timestamp}_{file.filename}" # Include original name for better traceability
        save_path = os.path.join(SERVER_IMAGE_STORAGE_ROOT, unique_filename)
        ...
        try:
            ...
            if not results_for_current_file:
                ...
                file_processing_result["file_path"] = save_path # Add saved path to result
...
async def upload_images_for_preview(
    ...
    for file in files:
        ...
        temp_original_path = os.path.join(TEMP_IMAGE_STORAGE_ROOT, unique_original_filename)
        ...
        try:
            ...
            if review_data:
                ...
                file_result['detection_time'] = review_data['detection_time'].isoformat() # ISO format for transport
            ...
            logger.error(f"Error processing file {file.filename} for preview: {e}")
            if os.path.exists(temp_original_path):
    ...
    return UploadPreviewResponse(
        ...
        original_filename=", ".join([f.filename for f in files]), # Concatenate filenames if multiple
...
async def confirm_and_save_faces(
    data: List[ConfirmSaveRequest], # Expect a list of confirmations for each image
    face_service: FaceService = Depends(get_face_service)
):
    if not data:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No data provided for saving.")
